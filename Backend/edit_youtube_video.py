...
FIRST_INPUT = "Im going now to send you a text.\nI need you to summerize that text using only direct quotes from it.\ndo it with no fillers,  just quotes.\nmake it so when you combine the quotes it is:\n- simple\n- intersting\n- keeps the same meaning\nKeep it short\ncan you write the indexes in the original text of the words you quoted?"
...
class EditedVideos:
    ...
    def do_nlp(self):
        for ind in range(len(self.dfs)):
            df, tp = self.read_df(ind)
            self.dfs[ind] = df
            self.text_parts.append(tp)
            time_intervals, df = self.call_chat_gpt(ind) # Need to check what is the expected result from the call_chat_gpt answer and how to connect this part of the code to the rest
    ...
    def get_time_intervals(df, assistant_reply, text_part):
        lst = assistant_reply.replace("...", "").split('"')
        ...
        for x in short_list:
            qoute = x.split(' ')
            flag = True
            for i in range(len(df) - len(qoute)):
                to_compare = ' '.join(' '.join(df['words'].iloc[i:len(qoute) + i]).split(' ')[:len(qoute)])

                if x.lower() in to_compare.lower() and i > curr_loc:
                    curr_loc = i
                    all_locs.append((i, len(qoute)))
                    flag = False
                    break
            if flag:
                print("Problem here")
        ...
        return time_intervals, ret_df.iloc[new_df_inds]


    def get_summeraztion_times(df):
        df, text_part = read_df(df)
        assistant_reply = call_chat_gpt(text_part)
        return get_time_intervals(df, assistant_reply, text_part)
