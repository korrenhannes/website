...
def listen_for_messages():
    # Initialize the subscriber client outside the Flask context
    ...
    def callback(message):
        with app.app_context():
            ...
            socketio.emit('progress_update', {'data': progress_update})
            print("Sent info to client")
            message.ack()
    ...
    while True:
        try:
            ...
            streaming_pull_future.result(timeout=300)  # Adjust timeout as needed
        except (TimeoutError, Exception) as e:
            print(f"An error occurred: {e}. Reconnecting...")
            time.sleep(5)  # Wait for a bit before retrying to avoid rapid reconnection attempts
