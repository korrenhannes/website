...
// Specific CORS configuration for /stream-video
app.get('/stream-video', helmet.crossOriginResourcePolicy({ policy: "cross-origin" }), (req, res) => {
  // Set CORS headers specifically for this route
  res.header('Access-Control-Allow-Origin', '*');
  res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');

  console.log('Received request to stream video');
  ...
  file.exists().then(data => {
    const exists = data[0];
    if (!exists) {
      console.error('Video file not found in the bucket');
      return res.status(404).send('Video not found');
    }

    console.log('Streaming video file');
    
    // Set the Content-Type header for the video stream
    res.writeHead(200, {
